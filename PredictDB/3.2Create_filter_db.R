...
setwd("./HapTWAS/PredictDB/Haplotype")

# Model_summaries Table -----
model_summaries <- read.table('./summary/' %&% tissue %&% '/' %&% tissue %&% '_Model_training_chr1_model_summaries.txt',header = T, stringsAsFactors = F)
tiss_summary <- read.table('./summary/' %&% tissue %&% '/' %&% tissue %&% '_Model_training_chr1_summary.txt', header = T, stringsAsFactors = F)
n_samples <- tiss_summary$n_samples
for (i in 2:22) {
  model_summaries <- rbind(model_summaries,
                           read.table('./summary/' %&% tissue %&% '/' %&% tissue %&% '_Model_training_chr' %&% as.character(i) %&% '_model_summaries.txt', header = T, stringsAsFactors = F))
  tiss_summary <- rbind(tiss_summary,
                        read.table('./summary/' %&% tissue %&% '/' %&% tissue %&% '_Model_training_chr' %&% as.character(i) %&% '_summary.txt', header = T, stringsAsFactors = F))
}
model_summaries <- rename(model_summaries, gene = gene_id)

# Create a database connection
conn <- dbConnect(drv = driver, './dbs/1kGP_' %&% tissue %&% '_ElasticNet_models.db')
...
# Weights Table -----
weights <- read.table('./weights/' %&% tissue %&% '/' %&% tissue %&% '_Model_training_chr1_weights.txt', header = T,stringsAsFactors = F)
for (i in 2:22) {
  weights <- rbind(weights,
                   read.table('./weights/' %&% tissue %&% '/' %&% tissue %&% '_Model_training_chr' %&% as.character(i) %&% '_weights.txt', header = T, stringsAsFactors = F))
}
...
# Sample_info Table ----
sample_info <- data.frame(n_samples = n_samples, population = '1kGP') # Provide the population info
...
unfiltered_db <- './dbs/1kGP_' %&% tissue %&% '_ElasticNet_models.db'
filtered_db <- './dbs/1kGP_' %&% tissue %&% '_ElasticNet_models_filtered_signif.db'
driver <- dbDriver("SQLite")
