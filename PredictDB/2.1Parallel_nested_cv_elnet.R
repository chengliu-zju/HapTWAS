...
# snp_annotation_filter
get_snp_annot <- function(snp_annot_file) {
  snp_annot <- read.table(snp_annot_file, header = T, stringsAsFactors = F)
  snp_annot
}

# genotype_filter
get_genotype <- function(genotype_file) {
  gt_df <- read.table(genotype_file, header = T, stringsAsFactors = F, row.names = 1)
  gt_df <- t(gt_df) %>% as.data.frame()
  gt_df
}

# gene_annotation_filter
...
# Linear Regression_scale_residuals_expression
...
# k-fold cv elastic-net models performance estimates
nested_cv_elastic_net_perf <- function(x, y, n_samples, n_train_test_folds, n_folds, alpha, samples) {
  ...
  # and for each fold, calculates a n_folds cross-validated elastic net model. Lambda parameter is
  ...
  train_test_fold_ids <- generate_fold_ids(n_samples, n_train_test_folds)
  for (test_fold in 1:n_train_test_folds) {
    ...
    cv_fold_ids <- generate_fold_ids(length(y_train), n_folds)
    y_pred <- tryCatch({
      # Fit model with training data.
      fit <- cv.glmnet(x_train, y_train, nfolds = n_folds, alpha = alpha, type.measure='mse', foldid = cv_fold_ids)
  ...
  list(R2_avg=R2_avg, R2_sd=R2_sd, rho_avg=rho_avg, rho_se=rho_se, rho_avg_squared=rho_avg_squared, zscore_est=zscore_est, zscore_pval=zscore_pval, pval_est=pval_est)
}
...
# main_function(call all functions above)
main <- function(snp_annot_file, gene_annot, genotype_file, expr_df, covariates_file,
                 chrom, prefix, tissue, i, maf=0.05, n_folds=10, n_train_test_folds=5,
                 seed=20240727, cis_window=1e6, alpha=0.5, null_testing=FALSE) {
  ...
  snp_annot <- get_snp_annot(snp_annot_file)
  ...
  # Prepare output data----
  model_summary_file <- './summary/' %&% tissue %&% "/" %&% chrom %&% "/" %&% tissue %&% '_' %&% prefix %&% '_chr' %&% chrom %&% '.' %&% genes[i] %&% '.model_summaries.txt'
  #model_summary_cols <- c('gene_id', 'gene_name', 'gene_type', 'alpha', 'n_snps_in_window', 'n_snps_in_model', 'lambda_min_mse',
  #'test_R2_avg', 'test_R2_sd', 'rho_avg', 'rho_se', 'rho_avg_squared', 'zscore_est', 'zscore_pval', 'nested_cv_fisher_pval',
  #'cv_R2_avg', 'cv_R2_sd', 'cv_rho_avg', 'cv_rho_se', 'cv_rho_avg_squared', 'cv_zscore_est', 'cv_zscore_pval', 'cv_pval_est',
  #'in_sample_R2', 'in_sample_rho', 'in_sample_rho_squared', 'in_sample_R2_avg', 'in_sample_rho_avg', 'in_sample_rho_squared_avg')
  #write(model_summary_cols, file = model_summary_file, ncol = 29, sep = '\t')
  
  weights_file <- './weights/' %&% tissue %&% "/" %&% chrom %&% "/" %&% tissue %&% '_' %&% prefix %&% '_chr' %&% chrom %&% '.' %&% genes[i] %&% '.weights.txt'
  #weights_col <- c('gene_id', 'rsid', 'varID', 'ref', 'alt', 'beta')
  #write(weights_col, file = weights_file, ncol = 6, sep = '\t')
  
  tiss_chr_summ_f <- './summary/' %&% tissue %&% "/" %&% chrom %&% "/" %&% tissue %&% '_' %&% prefix %&% '_chr' %&% chrom %&% '_summary.txt'
  ...
  covariance_file <- './covariances/' %&% tissue %&% "/" %&% chrom %&% "/" %&% tissue %&% '_' %&% prefix %&% '_chr' %&% chrom %&% '.' %&% genes[i] %&% '.covariances.txt'
  ...
  if (all(is.na(cis_gt))) {
    ...
    model_summary <- c(gene, gene_name, gene_type, alpha, 0, 0, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)
    write(model_summary, file = model_summary_file, ncol = 26, sep = '\t')
    #next
  } else {
    model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), 0, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA)
    if (ncol(cis_gt) >= 2) {
      ...
      zscore_est <- perf_measures$zscore_est
      ...
      if (length(fit) > 0) {
        ...
        training_rho <- ifelse(sd(adj_expr_pred) != 0, cor(adj_expr_pred, adj_expression), 0)
        training_rho_squared <- training_rho**2
        ...
        if (fit$nzero[best_lam_ind] > 0) {
          ...
          model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), fit$nzero[best_lam_ind], fit$lambda[best_lam_ind],
                             R2_avg, R2_sd, rho_avg, rho_se, rho_avg_squared, zscore_est, zscore_pval, pval_est,
                             cv_R2_avg, cv_R2_sd, cv_rho_avg, cv_rho_se, cv_rho_avg_squared, cv_zscore_est, cv_zscore_pval, cv_pval_est,
                             training_R2, training_rho, training_rho_squared)
        } else {
          model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), 0, fit$lambda[best_lam_ind],
                             R2_avg, R2_sd, rho_avg, rho_se, rho_avg_squared, zscore_est, zscore_pval, pval_est,
                             cv_R2_avg, cv_R2_sd, cv_rho_avg, cv_rho_se, cv_rho_avg_squared, cv_zscore_est, cv_zscore_pval, cv_pval_est,
                             training_R2, training_rho, training_rho_squared)
        }
      } else {
        model_summary <- c(gene, gene_name, gene_type, alpha, ncol(cis_gt), 0, NA,
                           R2_avg, R2_sd, rho_avg, rho_se, rho_avg_squared, zscore_est, zscore_pval, pval_est,
